...
export function pipe(...fns) {
  ...
  const leftToRightFn = (input) => {
    const result = fns.reduce((acc, fn) => fn(acc), input);
    console.log(result);
    return result;
  };
  return leftToRightFn;
}
...
export function compose(...fns) {
  ...
  const rightToLeftFn = (input) => {
    const result = fns.reduceRight((acc, fn) => fn(acc), input);
    console.log(result);
    return result;
  };
  return rightToLeftFn;
}
...
export function createRecipe(steps) {
  ...
  const validFunctions = steps
    .filter((step) => stepMap[step]) // keep only known steps
    .map((step) => stepMap[step]); // convert to functions
